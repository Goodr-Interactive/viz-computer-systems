      ...
      <Tabs defaultValue={Tab.ACTIVE} className="h-full w-full pb-[72px]">
        <TabsList className="mx-[12px] h-[36px] w-[376px]">
          <TabsTrigger value={Tab.ACTIVE}>
            Active Processes {active.length ? `(${active.length})` : ""}
          </TabsTrigger>
          <TabsTrigger value={Tab.COMPLETED}>
            Completed Processes {completed.length ? `(${completed.length})` : ""}
          </TabsTrigger>
        </TabsList>
        <TabsContent value={Tab.ACTIVE} className="h-full">
          <ScrollArea className="h-full px-[12px]">
            <div className="flex h-full w-full flex-col gap-[12px]">
              {active.filter(p => p.pid !== controller.lastRun?.pid || !controller.quizMode).map((p) => (
                <ProcessCard
                  key={p.pid}
                  process={p}
              ...
              {!active.length && (
                <div className="bg-grey flex h-full w-full items-center justify-center">
                  No Active Processes
                </div>
              )}
            </div>
