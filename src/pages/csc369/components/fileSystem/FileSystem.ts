...
export class FileSystem {
  ...
  public createFile(path: string, sizeOrContentType: number | "text" | "base64", content?: string): boolean {
  ...
  private createFileWithContent(path: string, contentType: "text" | "base64", content: string): boolean {
    ...
    for (let i = 0; i < this.superBlock.s_inodes_count; i++) {
      ...
      for (let j = 0; j < DIRECT_POINTERS; j++) {
        if (inode.i_block_pointers[j] === blockIndex) {
          ...
          const isEmptyOrZeros = contentBytes.length === 0 || contentBytes.every(byte => byte === 0);
          
          if (isEmptyOrZeros) {
            const loremText = faker.lorem.words(Math.floor(Math.random() * 20) + 10);
            return loremText;
          }
