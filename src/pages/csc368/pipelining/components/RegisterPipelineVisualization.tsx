...
export const RegisterPipelineVisualization: React.FC<RegisterPipelineVisualizationProps> = ({
  ...
  useEffect(() => {
      ...
      for (
        ...
        g.append("rect")
          ...
          .on("mouseover", function (event) {
            ...
            tooltip.append("text").attr("x", 10).attr("y", 20).text(`Instruction: ${instr.name}`);
            ...
            tooltip
              ...
              .attr("y", 60)
              .text(
                `Registers: ${instr.registers.src.join(", ")} → ${instr.registers.dest.join(", ")}`
              );
          })
    ...
    if (isPipelined && cycles > 0) {
      ...
      const registerLinesGroup = g.append("g").attr("class", "register-lines").style("opacity", 0);
  ...
  const handleAddInstruction = () => {
    ...
    if (
      ...
      instructionName.startsWith("slt")
    ) {
      ...
      instructionName.startsWith("lw") ||
      instructionName.startsWith("lb") ||
      instructionName.startsWith("lh") ||
      instructionName.startsWith("ld")
    ) {
      ...
      instructionName.startsWith("sw") ||
      instructionName.startsWith("sb") ||
      instructionName.startsWith("sh") ||
      instructionName.startsWith("sd")
    ) {
      ...
      instructionName.startsWith("beq") ||
      instructionName.startsWith("bne") ||
      instructionName.startsWith("blt") ||
      instructionName.startsWith("bge")
    ) {
      ...
      <div className="mb-2 flex w-full flex-col justify-between md:flex-row md:items-center">
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            <h3 className="text-lg font-medium">Current Cycle: {cycles}</h3>
          </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-green-700">
                <strong>All done!</strong> All instructions have been completed. It took {cycles}{" "}
                cycles to complete all {pipelineInstructions.length} instructions.
              </p>
            </div>
          </div>
        </div>
